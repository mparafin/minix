...
#define SCHED_RATE (MILLISEC*HZ/1000)	/* number of ticks per schedule for group A*/
#define SCHED_RATE_B  (2*SCHED_RATE)  /* number of ticks per schedule for group B*/
 ...
 */
  ...
  if(proc_ptr->p_group == 0) sched_ticks = SCHED_RATE;		/* reset quantum for group A*/
  if(proc_ptr->p_group == 1) sched_ticks = SCHED_RATE_B;  /* reset quantum for group B*/
