...
_PROTOTYPE( struct proc * find_user_proc_by_pid, (pid_t) );
 FORWARD _PROTOTYPE( int do_getprocgr, (message *m_ptr) );
 FORWARD _PROTOTYPE( int do_switchprocgr, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_GETPROCGR: r = do_getprocgr(&m); break;
      case SYS_SWITCHPROCGR: r = do_switchprocgr(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->p_group = 0; /* new process is by default in group A */
 ...
 *				do_getprocgr				     *
 *===========================================================================*/
PRIVATE int do_getprocgr(m_ptr)
message *m_ptr;
{
	struct proc * rp;
 ...
 *				do_switchprocgr				     *
 *===========================================================================*/
PRIVATE int do_switchprocgr(m_ptr)
message *m_ptr;
{
  struct proc* rp;
