 ...
 */
  ...
  if(rp->p_priority == PPRI_USER){
    prev_group = rp->p_group; /*the group of the last process that has been run */
  }
  else prev_group = -1;
  ...
  if(prev_group != 0){ /* if the last process was in the group B */
    if ( (rp = rdy_head[USER_Q]) != NIL_PROC) {
	    proc_ptr = rp;
	    bill_ptr = rp;
	    return;
    }
  else{               /* if the last process was in the group A */
    if ( (rp = rdy_head[USER_Q_B]) != NIL_PROC) {
	    proc_ptr = rp;
	    bill_ptr = rp;
	    return;
      }
 ...
 */
  ...
  if (rp->p_group == 0){
    ...
    if (rdy_head[USER_Q_B] == NIL_PROC)
	    rdy_tail[USER_Q_B] = rp;
    rp->p_nextready = rdy_head[USER_Q_B];
    rdy_head[USER_Q_B] = rp;
    /* CUR_B += 1; */
  }
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
    ...
    if (rp->p_group == 0) {       /* if the process is in group A */
  ...
  }       /* end of group A handling */

  if (rp->p_group == 1) {       /* if the process is in group B */
	if ( (xp = rdy_head[USER_Q_B]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[USER_Q_B] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
	...
	qtail = &rdy_tail[USER_Q_B];
  }       /* end of group B handling */

  }       /* end of user processes handling */
 ...
 */
  ...
  if (rdy_head[USER_Q] == NIL_PROC && rdy_head[USER_Q_B] == NIL_PROC) return;
  ...
  if(rp->p_group == 0){
    ...
    rdy_tail[USER_Q_B]->p_nextready = rdy_head[USER_Q_B];
    rdy_tail[USER_Q_B] = rdy_head[USER_Q_B];
    rdy_head[USER_Q_B] = rdy_head[USER_Q_B]->p_nextready;
    rdy_tail[USER_Q_B]->p_nextready = NIL_PROC;
  }
  pick_proc();
}
